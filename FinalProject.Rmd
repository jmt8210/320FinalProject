...
- _randomForest_ will allow us to use Random Forest modeling on our data
...
testTable2 <- teamGameStats %>%
  select(yardsFor, yardsAgainst, turnoverDiff, home, timeOfPossesionDifferential, Penalty, win)
team_data2 <- testTable %>%
  na.omit()
# Divide data set into 10 folds
fold_indices <- cvFolds(n=nrow(testTable), K = 10)
error_rates <- sapply(1:10, function(fold_index) {
...
avg_error_rates2 <- as.data.frame(t(error_rates)) %>%
  mutate(fold=1:n()) %>%
  gather(method,error,-fold) %>%
  # Get the average error rate
  summarize(error = mean(error))
avg_error_rates <- rbind(avg_error_rates, avg_error_rates2)
kable(avg_error_rates) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"))
```


## Logistic Prediction Error For Each Variable {.tabset}
