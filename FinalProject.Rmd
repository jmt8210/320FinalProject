...
# Don't ask why this doesn't work, I don't know. It's 3 in the morning so I'm not gonna worry about it.
#teamGameStats2013 %>%
#  mutate(opponentPoints = findPoints(as.character(teamGameStats2013$`Game Code`), as.double(teamGameStats2013$`Team #Code`)))
...
wins <- teamGameStats2013 %>%
  ...
  summarize(Wins=n())
wins %>%
  inner_join(passing2013, by="Team Code") %>%
  ggplot(aes(x=Wins, y=`Yards`)) + geom_point()
---------------------------------------------------
