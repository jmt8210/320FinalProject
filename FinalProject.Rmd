...
College Football has become the backbone of many college athletics programs. With greater revenue than the next 14 college sports _combined_, NCAA Football regually draws more fans and viewship than other major professional sports league.
...
- `tidyverse` will be used for tidying and manupulating data.frames
...
Next, we wanted to see how effective different progams were on run plays. To do this, we used a grouped together from 5 historically great college football teams... and Maryland, and compared their rushing yards per play with a violin plot. For this, we can notice that every school has a couple outlier, break-away plays on the run, which shouldn't really affect our view of each school's effectiveness on the run. However, if we look Ohio State's distribution, we can see they are sigificantly more effective in the 10-15 yard attempt range, showcasing their stronger running ability. This makes sense, especially with star running backs like Ezekiel Elliot, carlos Hyde, and Beanie Wells. However, 
...
rushing %>%
  ...
  mutate(standardizedRushAttempt = ((Yards-meanRushAttempt)/sdRushAttempt)) %>%
  inner_join(teams2013, by="Team Code") %>%
  ggplot(aes(x=reorder(factor(Name), standardizedRushAttempt), y=Yards, color=Name)) +
    geom_violin() +
    labs(x="",
         y="Rushing Yards",
...
```{r compareMethodError, warning=FALSE}
