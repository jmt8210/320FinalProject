...
With an error rate of .1914, Random Forest seems like a strong modeling option. However, this is very computationally intensive, taking 5-10 minutes on our PC to compute. 
...
Surprisingly, our logistic model has even better error than our random forest model, with an error of about .1907. This error, which not enough to to be statistically sigificant, is slightly better that our random forest error rate. Due to the much higher computational cost, and the conseqentual time penalty caused by our random forest modeling, we will be using logisitc regression to analyze our error rates from here on out.
